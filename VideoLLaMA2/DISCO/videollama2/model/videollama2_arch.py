...
class Videollama2MetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        clip_frame_features, mm_features = self.encode_images_or_videos(images)  # torch.Size([bs, 1024]) torch.Size([bs, 845, 4096])
        ...
        image_guide_features_mean = clip_frame_features.mean(dim=0, keepdim=True)
        ...
        # print(cos_sim_softmax)

        if self.training:
            current_image_features = clip_frame_features  # [batch_size, feature_dim]
            ...
            # print(self.global_text_feature[task_id])
