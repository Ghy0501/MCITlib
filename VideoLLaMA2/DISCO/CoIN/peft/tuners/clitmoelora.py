    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.disable_adapters:   # No adapter
            ...
            if self.training:
                ...
                dropout_x = self.lora_dropout[self.active_adapter](x.to(self.lora_A[self.active_adapter].loraA[0].mlp.weight.dtype))
                mask_tensor = torch.tensor(self.mask_signal, device=x.device, dtype=x.dtype)
                k = min(self.top_k, self.expert_num)
                topk_values, topk_indices = torch.topk(mask_tensor, k=k)
                # print(topk_values, topk_indices)
                
                sum_values = topk_values.sum()
                if sum_values > 1e-6:
                    normalized_weights = topk_values / sum_values
                else:
                    normalized_weights = topk_values

                lora_out_acc = 0.0
                
                for i in range(k):
                    expert_idx = topk_indices[i].item()
                    
                    lora_AB = normalized_weights[i]
                    
                    if lora_AB > 1e-6:
                        x_a = self.lora_A[self.active_adapter].loraA[expert_idx](dropout_x)
                        x_mixed = x_a * lora_AB
                        x_b = self.lora_B[self.active_adapter].loraB[expert_idx](x_mixed)
                        lora_out_acc += x_b

                result += lora_out_acc * self.scaling[self.active_adapter]
