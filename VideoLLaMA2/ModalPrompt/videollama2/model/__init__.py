...
def load_pretrained_model(model_path, model_base, model_name, prefix_len, cur_task, text_tower, num_tasks, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    if 'token' in kwargs:
        token = kwargs['token']
    else:
        token = None
    ...
    if 'lora' in model_name.lower() or 'qlora' in model_name.lower():
        ...
        if any(k.startswith('model.model.') for k in non_lora_trainables):
            non_lora_trainables = {(k[6:] if k.startswith('model.') else k): v for k, v in non_lora_trainables.items()}
        model.load_state_dict(non_lora_trainables, strict=False)
        # model.base_model.load_state_dict(non_lora_trainables, strict=True)
