...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, num_task=10, text_tower=None, **kwargs):
    if 'token' in kwargs:
        token = kwargs['token']
    else:
        token = None
