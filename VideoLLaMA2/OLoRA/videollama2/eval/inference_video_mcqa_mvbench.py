...
tasks = {
    "Action Sequence": ("action_sequence.json", "videos/", "video", True), # has start & end
    "Action Prediction": ("action_prediction.json", "videos/", "video", True), # has start & end
    "Action Antonym": ("action_antonym.json", "videos/", "video", False),
    "Fine-grained Action": ("fine_grained_action.json", "videos/", "video", False),
    "Unexpected Action": ("unexpected_action.json", "videos/", "video", False),
    "Object Existence": ("object_existence.json", "videos/", "video", False),
    "Object Interaction": ("object_interaction.json", "videos/", "video", True), # has start & end
    "Object Shuffle": ("object_shuffle.json", "videos/", "video", False),
    "Moving Direction": ("moving_direction.json", "videos/", "video", False),
    "Action Localization": ("action_localization.json", "videos/", "video", True),  # has start & end
    "Scene Transition": ("scene_transition.json", "videos/", "video", False),
    "Action Count": ("action_count.json", "videos/", "video", False),
    "Moving Count": ("moving_count.json", "videos/", "video", False),
    "Moving Attribute": ("moving_attribute.json", "videos/", "video", False),
    "State Change": ("state_change.json", "videos/", "video", False),
    # "Fine-grained Pose": ("fine_grained_pose.json", "nturgbd/", "video", False),
    "Character Order": ("character_order.json", "videos/", "video", False),
    "Egocentric Navigation": ("egocentric_navigation.json", "videos/", "video", False),
    "Episodic Reasoning": ("episodic_reasoning.json", "videos/", "video", True),  # has start & end, read frame
    "Counterfactual Inference": ("counterfactual_inference.json", "videos/", "video", False),
}
...
def mvbench_dump(vid, instruct, letters, options, output):
    ...
    output = output.replace('Answer', '')
    pred_answer = re.findall(rf'[\(,\ ]*[{letters[0]}-{letters[-1]}][\),\ ]*', output)
    try:
        find_flag = False
