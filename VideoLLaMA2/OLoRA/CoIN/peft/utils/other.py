...
def prepare_model_for_kbit_training(model, use_gradient_checkpointing=True):
    ...
    if loaded_in_kbit and use_gradient_checkpointing:
        ...
        # DeepSpeed/ZeRO + re-entrant checkpointing can trigger "parameter marked ready twice"/double-gradient issues.
        # Prefer non-reentrant checkpointing when supported.
        try:
            model.gradient_checkpointing_enable(gradient_checkpointing_kwargs={"use_reentrant": False})
        except TypeError:
            # Older HF versions may not accept gradient_checkpointing_kwargs.
