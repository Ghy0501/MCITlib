...
def build_router_user_message(raw_question: str) -> str:
    """Wrap the dataset question with routing instructions (image tokens are added separately)."""
    q = (raw_question or "").strip()
    return f"{ROUTING_PROMPT}{q}\n{PROMPT_AFTER_QUESTION}"
...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        qs = build_router_user_message(line["text"])
...
def eval_model(args):
    ...
    for (input_ids, image_tensor), line in tqdm(zip(data_loader, questions), total=len(questions)):
        ...
        cur_prompt = build_router_user_message(line["text"])
