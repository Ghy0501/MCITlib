...
import sys
sys.path.append('/your_path/MCITlib_v3/InternVL/SMoLoRA')
import os
import copy
...
from CoIN.peft import PeftModel, TaskType, get_peft_model, SMoLoraConfig, WEIGHTS_NAME, set_peft_model_state_dict
...
class ModelArguments:
    ...
    ins_type: Optional[int] = field(default=0)
    ins_emb: Optional[str] = field(default=False)
...
def find_all_linear_names(model):
    ...
    for name, module in model.named_modules():
        if isinstance(module, cls):
            ...
            # PEFT matches with key.endswith(target_key). Names like "0" from nn.Sequential
            # would also match transformer layer paths ending in ".0", ".10", etc., and then
            # try to wrap whole LlamaDecoderLayer — only Linear/Conv1D are supported.
            if last.isdigit():
                continue
            lora_module_names.add(last)
