...
ANNO_PATH=$(read_config "$DATA_CONFIG" anno_path)
IMAGE=$(read_config "$DATA_CONFIG" test_folder)
RESULT_PATH=$(read_config "$DATA_CONFIG" result_path)
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.CoIN.model_vqa \
...
python scripts/convert_seed_for_submission.py \
    --annotation-file $ANNO_PATH \
    --result-file $output_file \
    --result-upload-file $RESULT_DIR/$STAGE/merge_for_submission.jsonl \
