...
import sys
sys.path.append('/your_path/MCITlib_v3/InternVL/DISCO')
import os
import copy
...
def train(attn_implementation=None):
    ...
    if training_args.lora_enable:
        ...
        # print('global_image_feature: {}'.format(torch.cat([param for param in model.global_image_feature], dim=0).float().detach().cpu().numpy()))
        # print('global_text_feature: {}'.format(torch.cat([param for param in model.global_text_feature], dim=0).float().detach().cpu().numpy()))
    else:
