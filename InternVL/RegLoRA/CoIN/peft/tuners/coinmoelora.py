...
class CoINMOELoraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        self.set_expert_trainable(adapter_name)
    ...
    def set_expert_trainable(self, adapter_name):
        if adapter_name in self.lora_A.keys():
            for i in range(self.expert_num):
                if i == self.cur_task:
                    self.lora_A[adapter_name].loraA[i].mlp.weight.requires_grad = True
                    self.lora_B[adapter_name].loraB[i].mlp.weight.requires_grad = True
                else:
                    self.lora_A[adapter_name].loraA[i].mlp.weight.requires_grad = False
                    self.lora_B[adapter_name].loraB[i].mlp.weight.requires_grad = False
