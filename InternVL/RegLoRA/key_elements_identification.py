...
for model_path in model_path_list:
    ...
    ori_param_set = set()
    for k in lora_params.keys(): # k: base_model.model.model.layers.30.mlp.down_proj.lora_B.default.loraB.1.weight
        ori_param = k.replace(".lora_A", "").replace(".lora_B", "") 
        ori_param = re.sub(r"\.lora[AB](\.\d+)?", "", ori_param)  # ori_param: base_model.model.model.layers.30.mlp.down_proj.weight
        ori_param_set.add(ori_param)
    lora_pairs = {k: [] for k in ori_param_set}
    for k in lora_params.keys():
        if f".loraA.{step}" in k or f".loraB.{step}" in k:
            temp_param = k.replace(".lora_A", "").replace(".lora_B", "")
            temp_param = re.sub(r"\.lora[AB](\.\d+)?", "", temp_param)
            lora_pairs[temp_param].append(k)
            if len(lora_pairs[temp_param]) == 2:
                lora_pairs[temp_param].sort()
