...
class CoINMOELoraLayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A.update(nn.ModuleDict({adapter_name: CoINMOELinearA(self.in_features, r, self.expert_num, self.cur_task, self.training, self.layer, self.expert_weight)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: CoINMOELinearB(r, self.out_features, self.expert_num, self.cur_task, self.training, self.layer, self.expert_weight)}))
...
class CoINMOELoraLinear(nn.Linear, CoINMOELoraLayer):
    def __init__(
        ...
        **kwargs,
        ...
        # --- 改进部分：预计算 Top-K 索引和权重 ---
        self.topk_indices = []
        self.topk_weights = []
        
        # 仅在推理模式、第31层、且有专家权重时执行 Top-K 筛选
        if not self.training and int(self.layer) == 31 and len(self.expert_weight) > 0:
            # 转换为 tensor 方便处理
            weights_tensor = torch.tensor(self.expert_weight, dtype=torch.float32)
            
            # 防止 k 大于实际专家数量
            k = min(topk, len(weights_tensor))
            
            # 获取前 k 个最大值和索引
            top_values, top_indices = torch.topk(weights_tensor, k)
            
            # 归一化权重 (Softmax 风格，保证和为1，防止数值过大或过小)
            # 加上 1e-6 防止除以 0
            normalized_weights = top_values / (top_values.sum() + 1e-6)
            
            self.topk_indices = top_indices.tolist()
            self.topk_weights = normalized_weights.tolist()
        # ---------------------------------------
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        if self.disable_adapters:
            ...
            # 这里的 x 类型转换保留，用于输入 LoRA
            x_lora_input = x.to(self.lora_A[self.active_adapter].loraA[0].weight.dtype)

            if self.training:
                # 训练逻辑保持不变
                lora_a_output = self.lora_A[self.active_adapter].loraA[self.cur_task](self.lora_dropout[self.active_adapter](x_lora_input))
                ...
                if int(self.layer) != 31:
                    # 非 Top Layer：普通 LoRA 逻辑
                    lora_a_output = self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x_lora_input))
                    lora_b_output = self.lora_B[self.active_adapter](lora_a_output)
                    result += lora_b_output * self.scaling[self.active_adapter]
                else:
                    moe_accumulator = torch.zeros_like(result, dtype=torch.float32)
                    
                    x_input_f32 = x_lora_input.to(torch.float32)
                    dropout_x = self.lora_dropout[self.active_adapter](x_input_f32)

                    if len(self.topk_indices) > 0:
                        for idx, weight in zip(self.topk_indices, self.topk_weights):
                            # 取出对应的 LoRA 层
                            layer_a = self.lora_A[self.active_adapter].loraA[idx]
                            layer_b = self.lora_B[self.active_adapter].loraB[idx]
                            
                            out_a = layer_a(dropout_x)
                            out_b = layer_b(out_a)
                    ...
                    result += moe_accumulator.to(result.dtype)
