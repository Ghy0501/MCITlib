...
class CLIPTextTower(nn.Module):
    ...
    def forward(self, text_inputs, return_hidden_states=False):
        # Fix: Check loading status
        if not self.is_loaded:
            self.load_model()
            # Move to correct device based on inputs
            if isinstance(text_inputs, dict) and 'input_ids' in text_inputs:
                self.text_tower.to(text_inputs['input_ids'].device)
            elif hasattr(text_inputs, 'input_ids'):
                self.text_tower.to(text_inputs.input_ids.device)
