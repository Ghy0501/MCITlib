...
import sys
sys.path.append('/your_path/MCITlib_v3/Video-LLaVA/OLoRA')
import os
import copy
import random
...
def train():
    ...
    # DeepSpeed ZeRO + gradient checkpointing can fail with re-entrant checkpointing.
    # Ensure Trainer uses non-reentrant checkpointing consistently.
    # Older transformers TrainingArguments has no gradient_checkpointing_kwargs field.
    ...
    if training_args.gradient_checkpointing:
        # Also enable non-reentrant checkpointing directly on the model (robustness).
        try:
